                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-group select-style">
                            ...
                            <select name="experience_level" id=""
                                class="{{ $errors->has('experience_level') ? 'is-invalid' : '' }} form-icons select-active">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('website') ? 'is-invalid' : '' }}" type="text"
                                value="{{ $candidate?->website }}" name="website" placeholder="eg:www.example.com">
                            <x-input-error :messages="$errors->get('website')" class="mt-2" />
                        </div>
                    </div>
                    {{-- Yihew wendeme ye Age validation ke 21 amet eske 60 amet yehone sew becha nw masgebat yemicilew --}}
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="font-sm color-text-muted mb-10">Date of Birth</label>
                            <?php
                            $today = date('Y-m-d');
                            $maxDate = date('Y-m-d', strtotime('-21 years', strtotime($today)));
                            $minDate = date('Y-m-d', strtotime('-60 years', strtotime($today)));
                            ?>
                            <input type="date" name="date_of_birth"
                                class="form-control {{ $errors->has('date_of_birth') ? 'is-invalid' : '' }}"
                                value="{{ $candidate?->birth_date }}" min="{{ $minDate }}"
                                max="{{ $maxDate }}">
